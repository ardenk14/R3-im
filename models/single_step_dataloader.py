...
class FetchMotionDataset(Dataset):
    ...
    def __init__(self, data_folder, num_actions=5):
        ...
        # move to tensors
        self.q1 = torch.tensor(self.data['q1'], device=self.device)
        self.r3m1 = torch.tensor(self.data['r3m1'], device=self.device)
        self.g1 = torch.tensor(self.data['g1'], device=self.device)
        self.q2 = torch.tensor(self.data['q2'], device=self.device)
        self.g2 = torch.tensor(self.data['g2'], device=self.device)
    ...
    def __getitem__(self, item):
        ...
        sample = {
            ...
            'joint_state': torch.cat(self.q1[index, run], self.g1[index, run]),
            'true_action': torch.cat(self.q2[index, run] - self.q1[index,run], self.g2[index, run])
        }
